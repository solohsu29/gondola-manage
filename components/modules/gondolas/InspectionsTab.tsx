  ...
  const handleScheduleInspection = async () => {
    if (!form.inspectionType || !form.inspectionDate || !form.inspectionTime) {
        ...
        toast.error("Missing required fields", {
            description: "Please fill in all required fields to schedule an inspection.",
...
const columns: ColumnDef<Inspection>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className={`px-2 py-1 text-xs font-medium rounded-full ${row.original.priority === 'urgent' ? 'bg-red-100 text-red-800' : row.original.priority === 'high' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'}`}>
        {row.original.priority ? row.original.priority.charAt(0).toUpperCase() + row.original.priority.slice(1) : 'Normal'}
      </span>
    ),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (inspDate && inspDate > now) {
        return <span className="text-blue-600 font-medium bg-blue-100 px-2 py-1 rounded-full">Scheduled</span>;
      } else {
        return <span className="text-green-700 font-medium bg-green-100 px-2 py-1 rounded-full">Completed</span>;
      }
    },
  },
  {
            ...
            <Button
              onClick={() => {
                ...
                setEditForm({
                  ...
                  notes: insp.notes || '',
                  notifyClient: typeof insp.notifyClient === 'string' ? insp.notifyClient : (insp.notifyClient ? 'true' : 'false'),
                });
...
const handleEditInspection = async () => {
  ...
  ...payload,
    ...
    if (ok) {
      toast.success('Inspection updated!', { className: 'bg-[#14A44D] text-white' });
      setIsEditDialogOpen(false);
      setSelectedInspection(null);
    } else {
      toast.error('Failed to update inspection', { className: 'bg-destructive text-destructive-foreground' });
    }
  } catch (err: any) {
    toast.error('Failed to update inspection', { description: err.message, className: 'bg-destructive text-destructive-foreground' });
  }
};
      ...
      <CardContent className="p-0">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                ...
                <DialogDescription>Schedule a new inspection for {gondolaId}</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="space-y-2">
                  <Label htmlFor="inspectionType">Inspection Type *</Label>
                  <Select name="inspectionType" value={form.inspectionType} onValueChange={v => handleFormChange('inspectionType', v)}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="inspectionDate" type="date" min={new Date().toISOString().split("T")[0]} required value={form.inspectionDate} onChange={e => handleFormChange('inspectionDate', e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="inspectionTime">Inspection Time *</Label>
                  <Input id="inspectionTime" type="time" required value={form.inspectionTime} onChange={e => handleFormChange('inspectionTime', e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="inspector">Inspector</Label>
                  <Select name="inspector" value={form.inspector} onValueChange={v => handleFormChange('inspector', v)}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select name="priority" value={form.priority} onValueChange={v => handleFormChange('priority', v)}>
                    <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="inspectionNotes" placeholder="Any special requirements or notes for this inspection" value={form.notes} onChange={e => handleFormChange('notes', e.target.value)} />
                </div>
              </div>
              <DialogFooter>
        ...
        <div className="overflow-x-auto">
          ...
          {(() => {
            ...
            if (inspectionsError) {
              return <div className="p-4 text-center text-red-600">{inspectionsError}</div>;
            }
            if (inspections.length === 0) {
              return <div className="p-4 text-center">No inspections found.</div>;
            }
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>Edit inspection details for {selectedInspection?.id}</DialogDescription>
            </DialogHeader>
            {selectedInspection && (
  <div className="grid gap-4 py-4">
    <div className="space-y-2">
      <Label htmlFor="editInspectionType">Inspection Type *</Label>
      <Select name="editInspectionType" value={editForm.type} onValueChange={v => setEditForm(f => ({ ...f, type: v }))}>
        <SelectTrigger>
          <SelectValue placeholder="Select inspection type" />
        </SelectTrigger>
        <SelectContent>
    ...
    <div className="space-y-2">
      ...
      />
                ...
                <div className="space-y-2">
                  ...
                  <Select name="editInspector" value={editForm.inspector} onValueChange={v => setEditForm(f => ({ ...f, inspector: v }))}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select inspector" />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select name="editPriority" value={editForm.priority} onValueChange={v => setEditForm(f => ({ ...f, priority: v }))}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select priority" />
                    </SelectTrigger>
                    <SelectContent>
        ...
        <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>View complete inspection information for {viewInspection?.id}</DialogDescription>
            </DialogHeader>
            {viewInspection && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {(() => {
                      ...
                      if (inspDate && inspDate > now) {
                        return <p className="px-2 py-1 text-xs w-fit font-medium rounded-full bg-blue-100 text-blue-800">Scheduled</p>;
                      } else {
                        return <p className="px-2 py-1 text-xs w-fit font-medium rounded-full bg-green-100 text-green-800">Completed</p>;
                      }
                    })()}
                  </div>
                  <div className="space-y-2">
