...
export default function EditProjectPage() {
  ...
  const { gondolas, fetchGondolas, fetchDeliveryOrders, deliveryOrders, gondolasLoading, gondolasError, updateProject } = useAppStore();
  ...
  useEffect(() => {
    ...
    fetch(`/api/project/${projectId}`)
      .then(async res => {
        ...
        setPrimaryDOId(data.primaryDOId || (data.deliveryOrders && data.deliveryOrders[0]?.id) || null);
        setPrimaryGondolaId(data.primaryGondolaId || (data.gondolas && data.gondolas[0]?.id) || null);
        setLoading(false);
      })
      .catch(err => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Update the project via Zustand store
    setLoading(true);
    setError(null);
    try {
      await updateProject(projectId, {
        client: updatedProject.client,
        site: updatedProject.site,
        gondolas: updatedProject.gondolas,
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={formData.projectManagerId ? String(formData.projectManagerId) : ""}
                  onValueChange={(value) => handleChange("projectManagerId", Number(value))}
                >
                  <SelectTrigger>
                      ...
                      {availableDOs?.map((deliveryOrder) => (
                        ...
                        >
                          <input
                            ...
                            checked={
                              formData.deliveryOrders?.some((selected) => selected.id === deliveryOrder.id) || false
                            }
                            onChange={(e) => {
                              if (e.target.checked) {
                                setFormData((prev) => ({
                                ...
                                setFormData((prev) => ({
                                  ...prev,
                                  deliveryOrders:
                                    prev.deliveryOrders?.filter((selected) => selected.id !== deliveryOrder.id) || [],
                                }))
  ...
  <span className="text-sm text-gray-400 italic">No document attached</span>
                  ...
                  <div>
                    <h3 className="text-sm font-medium mb-4">
                      Selected Delivery Orders ({formData.deliveryOrders?.length || 0})
                    </h3>
                    <div className="max-h-64 overflow-y-auto border rounded-md bg-gray-50">
                      {formData.deliveryOrders && formData.deliveryOrders.length > 0 ? (
                          ...
                          {formData.deliveryOrders.map((deliveryOrder, index) => (
                            ...
                            >
                              ...
                              <Button
                                ...
                                variant="ghost"
                                size="sm"
                                onClick={() => handleRemoveDeliveryOrder(index)}
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            </div>
                    ...
                    {formData.deliveryOrders && formData.deliveryOrders.length > 0 && (
                      <div className="mt-3 p-3 bg-blue-50 rounded-md">
                        ...
                        <ul className="text-sm text-blue-800 space-y-1">
                          <li>• Total DOs: {formData.deliveryOrders.length}</li>
                          <li>
                            • Primary DO:{" "}
                            {primaryDOId
                              ? formData.deliveryOrders.find((d) => d.id === primaryDOId)?.number || "Not set"
                              : "Not set"}
                          </li>
                          <li>
                            • Additional DOs:{" "}
                            {primaryDOId ? formData.deliveryOrders.length - 1 : formData.deliveryOrders.length}
                          </li>
                        </ul>
                        {!primaryDOId && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    <div className="space-y-4">
                      <h3 className="text-sm font-medium">Available Gondolas</h3>
                      <Input
                        type="search"
                    ...
                    <div className="max-h-64 overflow-y-auto border rounded-md">
                      {(() => {
                        const filteredGondolas = availableGondolas
                          .filter((gondola) => {
                            const matchesSearch =
                              gondola.id.toLowerCase().includes(gondolaSearch.toLowerCase()) ||
                              gondola.serialNumber.toLowerCase().includes(gondolaSearch.toLowerCase()) ||
                              gondola.location.toLowerCase().includes(gondolaSearch.toLowerCase())
                        ...
                        return filteredGondolas.map((gondola) => (
                          ...
                          >
                            <input
                              ...
                              checked={formData.gondolas?.some((selected) => selected.id === gondola.id) || false}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setFormData((prev) => ({
                                    ...prev,
                                    gondolas: [...(prev.gondolas || []), gondola],
                                  }))
                                } else {
                                  setFormData((prev) => ({
                                    ...prev,
                                    gondolas: prev.gondolas?.filter((selected) => selected.id !== gondola.id) || [],
                                  }))
                  ...
                  <div>
                    <h3 className="text-sm font-medium mb-4">Selected Gondolas ({formData.gondolas?.length || 0})</h3>
                    <div className="max-h-64 overflow-y-auto border rounded-md bg-gray-50">
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {primaryGondolaId === gondola.id && (
                                      <span className="text-xs text-green-600 font-medium">Primary Gondola</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                    ...
                    {formData.gondolas && formData.gondolas.length > 0 && (
                      <div className="mt-3 p-3 bg-green-50 rounded-md">
                        <h4 className="text-sm font-medium text-green-900 mb-2">Gondola Summary</h4>
                        <ul className="text-sm text-green-800 space-y-1">
                          <li>• Total Gondolas: {formData.gondolas.length}</li>
                          <li>
                            • Primary Gondola:{" "}
                            {primaryGondolaId
                              ? formData.gondolas.find((g) => g.id === primaryGondolaId)?.id || "Not set"
                              : "Not set"}
                          </li>
                          <li>
                            • Additional Gondolas:{" "}
                            {primaryGondolaId ? formData.gondolas.length - 1 : formData.gondolas.length}
                          </li>
                        </ul>
                      </div>
                    )}
